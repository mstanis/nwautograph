...
def main():
    ...
    f_if_list_spine = list(
        range(int(f_if_range_spine[0]), int(f_if_range_spine[1]) + 1)
    )

    f_if_range_leaf = config["leaves"]["fabric_interfaces"].split("-")
    f_if_list_leaf = list(range(int(f_if_range_leaf[0]), int(f_if_range_leaf[1]) + 1))
